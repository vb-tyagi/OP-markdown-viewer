...
Without `--dir` or `--files` the server never reads or writes your markdown files; the browser does that. With them, the session API can read and overwrite only those files (names are matched against the fixed list, never taken as paths), refuses cross-origin and foreign-Host requests like the review endpoint, refuses a save if the file's modification time changed since it was opened unless the page explicitly forces it, writes through a temporary file in the same folder with an fsync and a rename, verifies the result byte for byte, and keeps the previous version in `.op-markdown-viewer-backups/` beside the file (newest 30). It picks a free port at startup rather than fighting another process for one.
